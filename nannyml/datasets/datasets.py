...
def load_synthetic_regression_dataset():
    """Loads the synthetic car price regression dataset provided for testing the NannyML package.

    Returns
    -------
    reference : pd.DataFrame
        A DataFrame containing reference period of synthetic regression dataset
    analysis : pd.DataFrame
        A DataFrame containing analysis period of synthetic regression dataset
    analysis_tgt : pd.DataFrame
        A DataFrame containing target values for the analysis period of synthetic regression dataset

    Examples
    --------
    >>> from nannyml.datasets import load_synthetic_regression_dataset
    >>> reference_df, analysis_df, analysis_targets_df = load_synthetic_regression_dataset()

    """
    reference = load_csv_file_to_df('regression_synthetic_v1_reference.csv')
    analysis = load_csv_file_to_df('regression_synthetic_v1_analysis.csv')
    analysis_gt = load_csv_file_to_df('regression_synthetic_v1_analysis_targets.csv')
