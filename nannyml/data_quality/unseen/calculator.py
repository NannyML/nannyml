...
from nannyml.thresholds import (
    ConstantThreshold, Threshold, calculate_threshold_values
)
...
class UnseenValuesCalculator(AbstractCalculator):
    ...
    def __init__(
        ...
        thresholds: Optional[Threshold] = ConstantThreshold(lower=None, upper=0),
    ):
    ...
    @log_usage(
        UsageEvent.DQ_CALC_UNSEEN_VALUES_FIT, metadata_from_self=['normalize']
    )
    def _fit(self, reference_data: pd.DataFrame, *args, **kwargs):
        ...
        for column in self.column_names:
            ...
            reference_chunk_results = np.asarray([
                self._calculate_unseen_value_stats(
                    chunk.data[column], _seen_values
    ...
    @log_usage(
        UsageEvent.DQ_CALC_UNSEEN_VALUES_RUN, metadata_from_self=['normalize']
    )
