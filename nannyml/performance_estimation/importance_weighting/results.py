...
from nannyml.performance_estimation.importance_weighting import SUPPORTED_METRIC_FILTER_VALUES
from nannyml.performance_estimation.importance_weighting.metrics import Metric
...
class Result(PerMetricResult[Metric], ResultCompareMixin):
    ...
    def __init__(
        ...
        """IW Performance Estimation results class.

        Parameters
        ----------
        results_data: pd.DataFrame
            Results data returned by a IW estimator.
        ...
        problem_type: ProblemType
            Determines which IW implementation to use. Allowed problem type values are 'classification_binary' and
    ...
    def keys(self) -> List[Key]:
        """Creates a list of keys. Each Key is a `namedtuple('Key', 'properties display_names')`."""
        return [
            Key(
    ...
    @log_usage(UsageEvent.IW_PLOT, metadata_from_kwargs=['kind'])
    def plot(
        self,
        ...
        **kwargs,
        ...
        """Render plots based on IW estimation results.
        ...
        Parameters
        ----------
        kind: Optional[str], default='performance'
