...
from nannyml.drift.univariate.methods import WassersteinDistance
...
def test_js_for_0_distance():
    ...
    js = JensenShannonDistance(chunker = None)
    js.fit(reference)
...
def test_wasserstein_both_continuous_0_distance():
    ...
    reference = pd.Series(np.random.normal(0, 1, 10_000))
    analysis = reference
    wass_dist = WassersteinDistance(chunker = None).fit(reference).calculate(analysis)
